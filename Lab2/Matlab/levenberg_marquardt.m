...
[x1, y1, f1_values, k1] = lm_method(f, gradf, hessianf, 'constant', 0, 0, 0.0001);
...
[x2, y2, f2_values, k2] = lm_method(f, gradf, hessianf, 'constant', -1, 1, 0.0001);
figure
for i=1:1:k2
...
[x3, y3, f3_values, k3] = lm_method(f, gradf, hessianf, 'constant', 1, -1, 0.0001);
figure
for i=1:1:k3
...
%------------------------------------------------------------------------------------------------------%Armijo%-------------------------------------------------------------------------------------------------------------------%
[x4, y4, f4_values, k4] = lm_method(f, gradf, hessianf, 'armijo', 0, 0, 0.0001);
...
[x5, y5, f5_values, k5] = lm_method(f, gradf, hessianf, 'armijo', -1, 1, 0.0001);
figure
for i=1:1:k5
...
[x6, y6, f6_values, k6] = lm_method(f, gradf, hessianf, 'armijo', 1, -1, 0.0001);
figure
for i=1:1:k6
...
[x7, y7, f7_values, k7] = lm_method(f, gradf, hessianf, 'minimization', 0, 0, 0.0001);
...
[x8, y8, f8_values, k8] = lm_method(f, gradf, hessianf, 'minimization', -1, 1, 0.0001);
figure
for i=1:1:k8
    plot(i, f5_values(i),'ob','MarkerSize', 5)
    hold on
...
[x9, y9, f9_values, k9] = lm_method(f, gradf, hessianf, 'minimization', 1, -1, 0.0001);
figure
for i=1:1:k9
...
%--------------------------------------------------Levenberg-Marquardt---------------------------------------------------------------------------%
function [x, y, f_values, k] = lm_method(f, gradf, hessianf, g_choice, initial_x, initial_y, e)
   ...
   isposdef = all(d > 0);
   if (isposdef == 0)
       msg = 'Hessian of f not positive difinite.';
       disp(msg);
       method = 1;
   end
    
   if(method == 1) %Levenberg-Marquardt method for non positive definite hessian matrix of f(x,y) 
       while (norm(gradf(x(k),y(k))) >= e)

           mk = 1;
           posdef_matrix = hessianf(x(k), y(k)) + (mk * [1 0; 0 1]);
           eigen = eig(posdef_matrix);
           isposdef = all(eigen > 0);
           ...
           dk_values(:, k) = inv((hessianf(x(k), y(k))) + (mk * [1 0; 0 1])) * (-gradf(x(k), y(k)));
           ...
           elseif (strcmp(g_choice,'minimization'))
                 ...
                 [a, b] = bisection_derivatives(h, 0, 5, 0.005);
